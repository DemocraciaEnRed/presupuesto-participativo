...
exports.populateTopics = function populateTopics (topics) {
  ...
  /*return Promise.all(topics.map(topic =>
    apiV1.user.getFullUserById(topic.owner).then(fullUser => {
      topic.owner = fullUser
      return topic
    })
  ))*/
  ...
  return find({ reference: { $in: topicIds } })
    ...
    .then((comments) =>
      Promise.all(comments.map(comment =>
        apiV1.user.populateUser(comment.author, true).then(() =>
          Promise.all(comment.replies.map(reply =>
            apiV1.user.populateUser(reply.author, true)
          ))
        ).then(() => comment)
      ))//end Promise.all
      ...
      comments.forEach((comment) => {
        ...
        let pushComment = scopes.ordinary.expose(comment)
        pushComment.author.lastName = comment.author.lastName
        pushComment.author.email = comment.author.email
        pushComment.replies = pushComment.replies.map((reply, i) => {
          reply.author.lastName = comment.replies[i].author.lastName
          reply.author.email = comment.replies[i].author.email
          return reply
        })
        topics[topicIn].comments.push(pushComment)
      })
